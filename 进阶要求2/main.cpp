...
static void HashSerializedGuesses(const string &buffer, double &time_hash, long long &local_hashed_total)
{
    ...
    auto flush_neon_batch = [&]() {
        ...
        {
            for (int i = 0; i < lane; i += 1)
            {
                MD5Hash(neon_inputs[i], neon_states[i]);
            }
        }
    ...
    for (int i = 0; i < count; i += 1)
    {
        ...
        if (length >= 56)
        {
            ...
            string guess(buffer.data() + offset, length);
            MD5Hash(guess, state0);
        }
        else
        {
            neon_inputs[lane].assign(buffer.data() + offset, length);
            lane += 1;
