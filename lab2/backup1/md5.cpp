...
void MD5Hash_NEON(string inputs[4], bit32 *state[4])
{
    for (int lane = 0; lane < 4; lane++)
    {
        if (inputs[lane].length() >= 56)
        {
            for (int i = 0; i < 4; i++)
            {
                MD5Hash(inputs[i], state[i]);
            }
            return;
        }
    }
    ...
    for (int lane = 0; lane < 4; lane++)
    {
        int len = inputs[lane].length();
        memcpy(blocks[lane], inputs[lane].c_str(), len);
        blocks[lane][len] = 0x80;

        uint64_t bit_len = (uint64_t)len * 8;
        for (int i = 0; i < 8; i++)
        {
            blocks[lane][56 + i] = (bit_len >> (8 * i)) & 0xff;
        }
    }
    ...
    for (int i = 0; i < 16; i++)
    {
        ...
        {
            word[lane] =
                ((uint32_t)blocks[lane][4 * i]) |
                ((uint32_t)blocks[lane][4 * i + 1] << 8) |
                ((uint32_t)blocks[lane][4 * i + 2] << 16) |
                ((uint32_t)blocks[lane][4 * i + 3] << 24);
        }
        x[i] = vld1q_u32(word);
    }
    ...
    a = vaddq_u32(a, vdupq_n_u32(0x67452301));
    b = vaddq_u32(b, vdupq_n_u32(0xefcdab89));
    c = vaddq_u32(c, vdupq_n_u32(0x98badcfe));
    d = vaddq_u32(d, vdupq_n_u32(0x10325476));
    ...
    for (int lane = 0; lane < 4; lane++)
    {
        state[lane][0] = byte_swap32(aa[lane]);
        state[lane][1] = byte_swap32(bb[lane]);
        state[lane][2] = byte_swap32(cc[lane]);
        state[lane][3] = byte_swap32(dd[lane]);
    }
}
