...
for (int lane = 0; lane < 4; lane++)
{
    size_t len = inputs[lane].length();
    const unsigned char *data =
        reinterpret_cast<const unsigned char *>(inputs[lane].data());

    for (size_t j = 0; j < len; j++)
    {
        words[j >> 2][lane] |=
            ((uint32_t)data[j]) << ((j & 3) * 8);
    }

    words[len >> 2][lane] |=
        ((uint32_t)0x80) << ((len & 3) * 8);

    words[14][lane] = (uint32_t)(len * 8);
    words[15][lane] = 0;
}
...
for (int i = 0; i < 16; i++)
{
    ...
    for (int lane = 0; lane < 4; lane++)
    {
		state[lane][0] = BYTE_SWAP32_NEON(aa[lane]);
		state[lane][1] = BYTE_SWAP32_NEON(bb[lane]);
		state[lane][2] = BYTE_SWAP32_NEON(cc[lane]);
		state[lane][3] = BYTE_SWAP32_NEON(dd[lane]);
    }
}
#endif
