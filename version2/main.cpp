...
static int MaxPendingSends()
{
    return ReadEnvInt("HYBRID_MAX_PENDING_SENDS", 4, 1, 64);
}
...
static void CleanupCompletedSends(list<PendingBatchSend> &pending_sends)
{
    for (auto it = pending_sends.begin(); it != pending_sends.end();)
    {
        int completed = 0;
        MPI_Testall(2, it->requests, &completed, MPI_STATUSES_IGNORE);
        if (completed)
        {
...
static void WaitOldestSend(list<PendingBatchSend> &pending_sends)
{
    ...
    MPI_Waitall(2, pending_sends.front().requests, MPI_STATUSES_IGNORE);
    pending_sends.pop_front();
}

static void WaitAllSends(list<PendingBatchSend> &pending_sends)
{
    for (PendingBatchSend &send : pending_sends)
    {
        MPI_Waitall(2, send.requests, MPI_STATUSES_IGNORE);
    }
    pending_sends.clear();
}

static void SendGuessBatchAsync(
    const vector<string> &guesses,
    int dest_rank,
    list<PendingBatchSend> &pending_sends)
{
    pending_sends.emplace_back();
    PendingBatchSend &send = pending_sends.back();

    send.buffer = SerializeGuesses(guesses);
    send.bytes = static_cast<int>(send.buffer.size());

    MPI_Isend(&send.bytes, 1, MPI_INT, dest_rank, TAG_BATCH_SIZE, MPI_COMM_WORLD, &send.requests[0]);
    MPI_Isend(send.buffer.data(), send.bytes, MPI_CHAR, dest_rank, TAG_BATCH_DATA, MPI_COMM_WORLD, &send.requests[1]);
}

static void FlushSendQueueIfNeeded(list<PendingBatchSend> &pending_sends, int max_pending_sends)
{
    CleanupCompletedSends(pending_sends);
    while ((int)pending_sends.size() >= max_pending_sends)
    {
        WaitOldestSend(pending_sends);
        CleanupCompletedSends(pending_sends);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    int next_hasher = 0;
    list<PendingBatchSend> pending_sends;
    int max_pending_sends = MaxPendingSends();
    ...
    while (!q.priority.empty())
    {
        ...
        if (global_guesses > batch_threshold)
        {
            if (hasher_count > 0 && !q.guesses.empty())
            {
                ...
                SendGuessBatchAsync(q.guesses, dest, pending_sends);
                FlushSendQueueIfNeeded(pending_sends, max_pending_sends);
