...
{
    if (priority.empty() || batch_size <= 0)
    {
        return;
    }
    ...
    if (actual_batch_size > (int)priority.size())
    {
        actual_batch_size = priority.size();
    }

    if (actual_batch_size > mpi_size)
    {
        actual_batch_size = mpi_size;
    }

    vector<PT> batch_pts;
    batch_pts.reserve(actual_batch_size);

    for (int i = 0; i < actual_batch_size; i += 1)
    {
        batch_pts.emplace_back(priority[i]);
    }

    if (mpi_rank < actual_batch_size)
    {
        GenerateFull(batch_pts[mpi_rank]);
    }
...
void PriorityQueue::GenerateFull(PT pt)
{
    CalProb(pt);
        ...
        else
        {
            ...
            for (int i = 0; i < pt.max_indices[last]; i += 1)
            {
                string temp = guess + a->ordered_values[i];
                guesses.emplace_back(temp);
            }

            total_guesses += pt.max_indices[last];
        }
    }
